...
def fetch_routes():
    try:
        conn = connect_db()
        cursor = conn.cursor()
        cursor.execute("SELECT route_id, origin_spaceport_id, destination_spaceport_id FROM route")
...
def insert_flight(flight_number, route_id, craft_id, departure_time, flight_time):
...
def show_window():
    ...
    route_map = {f"Route {rid} (From {o} to {d})": rid for rid, o, d in routes}
    ...
    def on_submit():
        ...
        route_id = route_map.get(route_var.get())
        craft_id = craft_map.get(craft_var.get())
        departure_time = entry_depart.get().strip()
        flight_time = entry_time.get().strip()
        insert_flight(flight_number, route_id, craft_id, departure_time, flight_time)
