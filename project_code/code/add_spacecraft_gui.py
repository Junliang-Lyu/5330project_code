...
# Insert a spacecraft type into database
def insert_spacecraft(type_name, capacity, max_range):
    ...
    try:
        ...
        cursor.execute(
            ...
            (type_name, int(capacity), int(max_range))
        )
        conn.commit()
        cursor.close()
        conn.close()
...
def show_window():
    ...
    Label(win, text="Capacity (Integer):").pack(pady=3)
    entry_capacity = Entry(win, width=35)
    entry_capacity.pack()

    Label(win, text="Range (Integer, max distance):").pack(pady=3)
